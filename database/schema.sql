-- ============================================================
-- EstudAI - schema canonico do prototipo
-- Banco alvo: MySQL/MariaDB
-- A IA orienta rotina, calendario e revisao semanal.
-- Exercicios e simulados usam apenas questoes aprovadas do banco.
-- ============================================================
...
USE vestibular_estudos;

SET FOREIGN_KEY_CHECKS = 0;

DROP TABLE IF EXISTS simulados_planejados_respostas;
DROP TABLE IF EXISTS respostas_exercicios_planejados;
DROP TABLE IF EXISTS simulado_respostas;
DROP TABLE IF EXISTS simulado_tentativas;
DROP TABLE IF EXISTS simulado_questoes;
DROP TABLE IF EXISTS simulados_planejados;
DROP TABLE IF EXISTS exercicios_planejados;
DROP TABLE IF EXISTS revisoes_conteudo_ia;
DROP TABLE IF EXISTS revisoes_semanais_ia;
DROP TABLE IF EXISTS plano_versoes;
DROP TABLE IF EXISTS eventos_calendario_estudai;
DROP TABLE IF EXISTS tarefas_estudo;
DROP TABLE IF EXISTS plano_estudo_itens;
DROP TABLE IF EXISTS planejamento_semanal_controle;
DROP TABLE IF EXISTS planos_estudo;
DROP TABLE IF EXISTS revisoes_programadas;
DROP TABLE IF EXISTS sessoes_estudo;
DROP TABLE IF EXISTS rotina_semanal;
DROP TABLE IF EXISTS anotacoes;
DROP TABLE IF EXISTS conquistas_usuario;
DROP TABLE IF EXISTS conquistas;
DROP TABLE IF EXISTS estatisticas_materia;
DROP TABLE IF EXISTS historico_estudos;
DROP TABLE IF EXISTS desempenho_diario;
DROP TABLE IF EXISTS respostas_usuario;
DROP TABLE IF EXISTS questoes_do_dia;
DROP TABLE IF EXISTS questoes_respostas_usuario;
DROP TABLE IF EXISTS questoes_alternativas;
DROP TABLE IF EXISTS alternativas;
DROP TABLE IF EXISTS questoes;
DROP TABLE IF EXISTS conteudos;
DROP TABLE IF EXISTS materias;
DROP TABLE IF EXISTS ia_historico;
DROP TABLE IF EXISTS notificacoes_usuario;
DROP TABLE IF EXISTS preferencias_estudo;
DROP TABLE IF EXISTS preferencias_usuario;
DROP TABLE IF EXISTS metas_semanais;
DROP TABLE IF EXISTS onboarding_respostas;
DROP TABLE IF EXISTS estudo_perfis;
...
CREATE TABLE schema_migrations (
  ...
  versao VARCHAR(120) NOT NULL UNIQUE,
...
CREATE TABLE preferencias_usuario (
  ...
  horario_lembrete TIME NOT NULL DEFAULT '08:00:00',
  criada_em DATETIME NOT NULL DEFAULT CURRENT_TIMESTAMP,
  atualizada_em DATETIME NULL DEFAULT NULL ON UPDATE CURRENT_TIMESTAMP,
  UNIQUE KEY uniq_preferencias_usuario (usuario_id),
  CONSTRAINT fk_pref_usuario FOREIGN KEY (usuario_id) REFERENCES usuarios(id) ON DELETE CASCADE
) ENGINE=InnoDB;

CREATE TABLE preferencias_estudo (
  id INT AUTO_INCREMENT PRIMARY KEY,
  usuario_id INT NOT NULL,
  preferencias_json LONGTEXT NULL,
  criada_em DATETIME NOT NULL DEFAULT CURRENT_TIMESTAMP,
  atualizada_em DATETIME NULL DEFAULT NULL ON UPDATE CURRENT_TIMESTAMP,
  UNIQUE KEY uniq_preferencias_estudo_usuario (usuario_id),
  CONSTRAINT fk_pref_estudo_usuario FOREIGN KEY (usuario_id) REFERENCES usuarios(id) ON DELETE CASCADE
) ENGINE=InnoDB;
...
CREATE TABLE notificacoes_usuario (
  ...
  KEY idx_notificacoes_usuario (usuario_id, lida, criada_em),
...
CREATE TABLE estudo_perfis (
  id INT AUTO_INCREMENT PRIMARY KEY,
  usuario_id INT NOT NULL,
  onboarding_completo TINYINT(1) NOT NULL DEFAULT 0,
  objetivo VARCHAR(80) NULL,
  modo_estudo VARCHAR(40) NULL,
  lingua_estrangeira VARCHAR(40) NULL,
  ...
  preferencia_estudo VARCHAR(40) NULL,
  meta_semanal VARCHAR(120) NULL,
  notificacoes TINYINT(1) NOT NULL DEFAULT 1,
  disponibilidade_json LONGTEXT NULL,
  reforcos_json LONGTEXT NULL,
  materias_base_json LONGTEXT NULL,
  perfil_json LONGTEXT NULL,
  criado_em DATETIME NOT NULL DEFAULT CURRENT_TIMESTAMP,
  atualizado_em DATETIME NULL DEFAULT NULL ON UPDATE CURRENT_TIMESTAMP,
  UNIQUE KEY uniq_estudo_perfis_usuario (usuario_id),
  CONSTRAINT fk_estudo_perfis_usuario FOREIGN KEY (usuario_id) REFERENCES usuarios(id) ON DELETE CASCADE
) ENGINE=InnoDB;
...
CREATE TABLE conteudos (
  ...
  created_at DATETIME NOT NULL DEFAULT CURRENT_TIMESTAMP,
  updated_at DATETIME NULL DEFAULT NULL ON UPDATE CURRENT_TIMESTAMP,
  UNIQUE KEY uniq_conteudos_materia_slug (materia, slug),
  KEY idx_conteudos_materia (materia_id, status),
  CONSTRAINT fk_conteudos_materia FOREIGN KEY (materia_id) REFERENCES materias(id) ON DELETE SET NULL
) ENGINE=InnoDB;
...
CREATE TABLE questoes (
  ...
  fonte VARCHAR(160) NOT NULL DEFAULT 'Base propria EstudAI',
  ano SMALLINT NULL,
  prova VARCHAR(160) NULL,
  enunciado TEXT NOT NULL,
  explicacao TEXT NULL,
  status ENUM('rascunho','aprovada','revisada','aprovado','revisado','ativo','arquivada') NOT NULL DEFAULT 'rascunho',
  created_at DATETIME NOT NULL DEFAULT CURRENT_TIMESTAMP,
  updated_at DATETIME NULL DEFAULT NULL ON UPDATE CURRENT_TIMESTAMP,
  KEY idx_questoes_filtro (status, materia, conteudo, dificuldade),
  KEY idx_questoes_area (area, status)
) ENGINE=InnoDB;
...
CREATE TABLE questoes_alternativas (
  ...
  CONSTRAINT fk_questoes_alternativas_questao FOREIGN KEY (questao_id) REFERENCES questoes(id) ON DELETE CASCADE
) ENGINE=InnoDB;

CREATE TABLE questoes_do_dia (
  id INT AUTO_INCREMENT PRIMARY KEY,
  data DATE NOT NULL,
  questao_id INT NOT NULL,
  UNIQUE KEY uniq_questao_dia (data, questao_id),
  KEY idx_questoes_do_dia_data (data),
  CONSTRAINT fk_questoes_do_dia_questao FOREIGN KEY (questao_id) REFERENCES questoes(id) ON DELETE CASCADE
) ENGINE=InnoDB;
...
CREATE TABLE respostas_usuario (
  ...
  UNIQUE KEY uniq_resposta_usuario_dia (usuario_id, questao_id, data),
  KEY idx_respostas_usuario_data (usuario_id, data),
  ...
  CONSTRAINT fk_respostas_usuario_alternativa FOREIGN KEY (alternativa_id) REFERENCES questoes_alternativas(id) ON DELETE SET NULL
) ENGINE=InnoDB;
CREATE TABLE questoes_respostas_usuario (
  id INT AUTO_INCREMENT PRIMARY KEY,
  usuario_id INT NOT NULL,
  questao_id INT NOT NULL,
  alternativa_id INT NULL,
  ...
  tempo_resposta INT NULL,
  created_at DATETIME NOT NULL DEFAULT CURRENT_TIMESTAMP,
  KEY idx_respostas_usuario (usuario_id, created_at),
  KEY idx_respostas_questao (questao_id),
  CONSTRAINT fk_questoes_respostas_usuario FOREIGN KEY (usuario_id) REFERENCES usuarios(id) ON DELETE CASCADE,
  CONSTRAINT fk_questoes_respostas_questao FOREIGN KEY (questao_id) REFERENCES questoes(id) ON DELETE CASCADE,
  CONSTRAINT fk_questoes_respostas_alternativa FOREIGN KEY (alternativa_id) REFERENCES questoes_alternativas(id) ON DELETE SET NULL
) ENGINE=InnoDB;

-- ============================================================
-- Historico, metas, estatisticas e conquistas
-- ============================================================

CREATE TABLE desempenho_diario (
  id INT AUTO_INCREMENT PRIMARY KEY,
  usuario_id INT NOT NULL,
  data DATE NOT NULL,
  ...
  criado_em DATETIME NOT NULL DEFAULT CURRENT_TIMESTAMP,
  atualizado_em DATETIME NULL DEFAULT NULL ON UPDATE CURRENT_TIMESTAMP,
  UNIQUE KEY uniq_desempenho_usuario_data (usuario_id, data),
...
CREATE TABLE estatisticas_materia (
  ...
  UNIQUE KEY uniq_estatistica_usuario_materia (usuario_id, materia),
  CONSTRAINT fk_estatistica_usuario FOREIGN KEY (usuario_id) REFERENCES usuarios(id) ON DELETE CASCADE
) ENGINE=InnoDB;
...
CREATE TABLE metas_semanais (
  ...
  meta_questoes INT NOT NULL DEFAULT 100,
  questoes_feitas INT NOT NULL DEFAULT 0,
  acertos INT NOT NULL DEFAULT 0,
  concluida TINYINT(1) NOT NULL DEFAULT 0,
  criada_em DATETIME NOT NULL DEFAULT CURRENT_TIMESTAMP,
  atualizada_em DATETIME NULL DEFAULT NULL ON UPDATE CURRENT_TIMESTAMP,
  UNIQUE KEY uniq_meta_usuario_semana (usuario_id, semana_inicio),
...
CREATE TABLE conquistas (
  ...
  icone VARCHAR(40) NOT NULL DEFAULT 'star',
  categoria ENUM('streak','questoes','desempenho','especial') NOT NULL DEFAULT 'especial',
  requisito INT NOT NULL DEFAULT 0,
  criada_em DATETIME NOT NULL DEFAULT CURRENT_TIMESTAMP
) ENGINE=InnoDB;
...
CREATE TABLE conquistas_usuario (
  ...
  UNIQUE KEY uniq_conquista_usuario (usuario_id, conquista_id),
  CONSTRAINT fk_conq_usuario FOREIGN KEY (usuario_id) REFERENCES usuarios(id) ON DELETE CASCADE,
  CONSTRAINT fk_conq_conquista FOREIGN KEY (conquista_id) REFERENCES conquistas(id) ON DELETE CASCADE
) ENGINE=InnoDB;
...
CREATE TABLE anotacoes (
  ...
  criada_em DATETIME NOT NULL DEFAULT CURRENT_TIMESTAMP,
  atualizada_em DATETIME NOT NULL DEFAULT CURRENT_TIMESTAMP ON UPDATE CURRENT_TIMESTAMP,
  UNIQUE KEY uniq_anotacao_usuario_questao (usuario_id, questao_id),
  CONSTRAINT fk_anotacoes_usuario FOREIGN KEY (usuario_id) REFERENCES usuarios(id) ON DELETE CASCADE,
  CONSTRAINT fk_anotacoes_questao FOREIGN KEY (questao_id) REFERENCES questoes(id) ON DELETE CASCADE
) ENGINE=InnoDB;
...
-- ============================================================
-- Planos, calendario, rotina e tarefas
-- ============================================================
...
CREATE TABLE planos_estudo (
  ...
  perfil_id INT NULL,
  titulo VARCHAR(160) NOT NULL,
  origem ENUM('ia','manual','erro') NOT NULL DEFAULT 'manual',
  status ENUM('ativo','substituido','arquivado','concluido') NOT NULL DEFAULT 'ativo',
  tipo_plano ENUM('semanal','anual','legado') NULL,
  escopo ENUM('semanal','anual','legado') NULL,
  resumo TEXT NULL,
  ...
  plano_json LONGTEXT NULL,
  criado_em DATETIME NOT NULL DEFAULT CURRENT_TIMESTAMP,
  atualizado_em DATETIME NULL DEFAULT NULL ON UPDATE CURRENT_TIMESTAMP,
  KEY idx_planos_usuario_status (usuario_id, status),
  KEY idx_planos_semana (usuario_id, semana_inicio, semana_fim),
  CONSTRAINT fk_planos_usuario FOREIGN KEY (usuario_id) REFERENCES usuarios(id) ON DELETE CASCADE,
  CONSTRAINT fk_planos_perfil FOREIGN KEY (perfil_id) REFERENCES estudo_perfis(id) ON DELETE SET NULL
) ENGINE=InnoDB;

CREATE TABLE planejamento_semanal_controle (
  id INT AUTO_INCREMENT PRIMARY KEY,
  usuario_id INT NOT NULL,
  plano_id INT NULL,
  semana_inicio DATE NOT NULL,
  semana_fim DATE NOT NULL,
  status ENUM('ativo','substituido','arquivado','finalizado') NOT NULL DEFAULT 'ativo',
  origem ENUM('ia','manual','revisao_domingo','cron') NOT NULL DEFAULT 'manual',
  criado_em DATETIME NOT NULL DEFAULT CURRENT_TIMESTAMP,
  atualizado_em DATETIME NULL DEFAULT NULL ON UPDATE CURRENT_TIMESTAMP,
  KEY idx_planejamento_usuario_semana (usuario_id, semana_inicio, status),
  CONSTRAINT fk_planejamento_usuario FOREIGN KEY (usuario_id) REFERENCES usuarios(id) ON DELETE CASCADE,
  CONSTRAINT fk_planejamento_plano FOREIGN KEY (plano_id) REFERENCES planos_estudo(id) ON DELETE SET NULL
) ENGINE=InnoDB;

CREATE TABLE plano_estudo_itens (
  id INT AUTO_INCREMENT PRIMARY KEY,
  ...
  tipo_atividade ENUM('teoria','questoes','revisao','simulado','resumo','misto','custom') NOT NULL DEFAULT 'custom',
  titulo VARCHAR(160) NOT NULL,
  descricao TEXT NULL,
  tempo_estimado INT NOT NULL DEFAULT 30,
  status ENUM('pendente','em_andamento','concluida','cancelada') NOT NULL DEFAULT 'pendente',
  ordem INT NOT NULL DEFAULT 0,
  criado_em DATETIME NOT NULL DEFAULT CURRENT_TIMESTAMP,
  atualizado_em DATETIME NULL DEFAULT NULL ON UPDATE CURRENT_TIMESTAMP,
  KEY idx_itens_plano (plano_id, ordem),
  KEY idx_itens_usuario_data (usuario_id, data_prevista),
  CONSTRAINT fk_itens_plano FOREIGN KEY (plano_id) REFERENCES planos_estudo(id) ON DELETE CASCADE,
...
CREATE TABLE tarefas_estudo (
  ...
  materia VARCHAR(100) NULL,
  conteudo VARCHAR(180) NULL,
  tipo ENUM('teoria','questoes','revisao','simulado','resumo','misto','custom') NOT NULL DEFAULT 'custom',
  data_prevista DATE NULL,
  hora_inicio TIME NULL,
  hora_fim TIME NULL,
  tempo_estimado INT NOT NULL DEFAULT 30,
  ...
  status ENUM('pendente','em_andamento','adiada','remarcada','concluida','cancelada') NOT NULL DEFAULT 'pendente',
  origem ENUM('ia','manual','reajuste','revisao_domingo','erro') NOT NULL DEFAULT 'manual',
  fonte_conteudo ENUM('plano','manual','banco') NOT NULL DEFAULT 'plano',
  metadata_json LONGTEXT NULL,
  concluida_em DATETIME NULL,
  criada_em DATETIME NOT NULL DEFAULT CURRENT_TIMESTAMP,
  atualizada_em DATETIME NULL DEFAULT NULL ON UPDATE CURRENT_TIMESTAMP,
  KEY idx_tarefas_usuario_data (usuario_id, data_prevista, status),
  KEY idx_tarefas_plano (plano_id, data_prevista),
  KEY idx_tarefas_item (item_id),
  CONSTRAINT fk_tarefas_usuario FOREIGN KEY (usuario_id) REFERENCES usuarios(id) ON DELETE CASCADE,
  CONSTRAINT fk_tarefas_plano FOREIGN KEY (plano_id) REFERENCES planos_estudo(id) ON DELETE SET NULL,
  CONSTRAINT fk_tarefas_item FOREIGN KEY (item_id) REFERENCES plano_estudo_itens(id) ON DELETE SET NULL
) ENGINE=InnoDB;

CREATE TABLE eventos_calendario_estudai (
  id INT AUTO_INCREMENT PRIMARY KEY,
  usuario_id INT NOT NULL,
  plano_id INT NULL,
  tarefa_id INT NULL,
  tipo ENUM('tarefa','exercicio','revisao','simulado','resumo') NOT NULL DEFAULT 'tarefa',
  titulo VARCHAR(160) NOT NULL,
  descricao TEXT NULL,
  conteudo VARCHAR(180) NULL,
  ...
  semana_inicio DATE NULL,
  semana_fim DATE NULL,
  status ENUM('pendente','concluido','atrasado','bloqueado','cancelado') NOT NULL DEFAULT 'pendente',
  metadata_json LONGTEXT NULL,
  criado_em DATETIME NOT NULL DEFAULT CURRENT_TIMESTAMP,
  atualizado_em DATETIME NULL DEFAULT NULL ON UPDATE CURRENT_TIMESTAMP,
  KEY idx_eventos_usuario_data (usuario_id, data_evento, status),
  KEY idx_eventos_tarefa (tarefa_id),
  CONSTRAINT fk_eventos_usuario FOREIGN KEY (usuario_id) REFERENCES usuarios(id) ON DELETE CASCADE,
  CONSTRAINT fk_eventos_plano FOREIGN KEY (plano_id) REFERENCES planos_estudo(id) ON DELETE SET NULL,
  CONSTRAINT fk_eventos_tarefa FOREIGN KEY (tarefa_id) REFERENCES tarefas_estudo(id) ON DELETE SET NULL
) ENGINE=InnoDB;
...
CREATE TABLE rotina_semanal (
  ...
  criado_em DATETIME NOT NULL DEFAULT CURRENT_TIMESTAMP,
  atualizado_em DATETIME NULL DEFAULT NULL ON UPDATE CURRENT_TIMESTAMP,
  KEY idx_rotina_usuario (usuario_id, dia_semana),
  CONSTRAINT fk_rotina_usuario FOREIGN KEY (usuario_id) REFERENCES usuarios(id) ON DELETE CASCADE
) ENGINE=InnoDB;
...
CREATE TABLE sessoes_estudo (
  ...
  criado_em DATETIME NOT NULL DEFAULT CURRENT_TIMESTAMP,
  KEY idx_sessoes_usuario_inicio (usuario_id, inicio),
  CONSTRAINT fk_sessoes_usuario FOREIGN KEY (usuario_id) REFERENCES usuarios(id) ON DELETE CASCADE,
  CONSTRAINT fk_sessoes_tarefa FOREIGN KEY (tarefa_id) REFERENCES tarefas_estudo(id) ON DELETE SET NULL
) ENGINE=InnoDB;

CREATE TABLE revisoes_programadas (
  id INT AUTO_INCREMENT PRIMARY KEY,
  usuario_id INT NOT NULL,
  tarefa_id INT NULL,
  materia VARCHAR(100) NULL,
  ...
  criado_em DATETIME NOT NULL DEFAULT CURRENT_TIMESTAMP,
  atualizado_em DATETIME NULL DEFAULT NULL ON UPDATE CURRENT_TIMESTAMP,
  KEY idx_revisoes_programadas_usuario (usuario_id, data_revisao, status),
  CONSTRAINT fk_revisoes_programadas_usuario FOREIGN KEY (usuario_id) REFERENCES usuarios(id) ON DELETE CASCADE,
  CONSTRAINT fk_revisoes_programadas_tarefa FOREIGN KEY (tarefa_id) REFERENCES tarefas_estudo(id) ON DELETE SET NULL
) ENGINE=InnoDB;

-- ============================================================
-- IA controlada: diagnostico, plano, calendario e revisao semanal
-- ============================================================
...
CREATE TABLE ia_historico (
  ...
  criado_em DATETIME NOT NULL DEFAULT CURRENT_TIMESTAMP,
  KEY idx_ia_usuario_tipo (usuario_id, tipo, criado_em),
  CONSTRAINT fk_ia_usuario FOREIGN KEY (usuario_id) REFERENCES usuarios(id) ON DELETE SET NULL
) ENGINE=InnoDB;

CREATE TABLE revisoes_semanais_ia (
  id INT AUTO_INCREMENT PRIMARY KEY,
  usuario_id INT NOT NULL,
  plano_id INT NULL,
  semana_inicio DATE NOT NULL,
  semana_fim DATE NOT NULL,
  origem ENUM('manual','cron','ia') NOT NULL DEFAULT 'manual',
  tarefas_total INT NOT NULL DEFAULT 0,
  tarefas_concluidas INT NOT NULL DEFAULT 0,
  tarefas_atrasadas INT NOT NULL DEFAULT 0,
  percentual_conclusao DECIMAL(5,2) NOT NULL DEFAULT 0,
  minutos_planejados INT NOT NULL DEFAULT 0,
  minutos_concluidos INT NOT NULL DEFAULT 0,
  analise_json LONGTEXT NULL,
  ajuste_tipo ENUM('sem_ajustes','pequenos_ajustes','grandes_ajustes','recriacao') NOT NULL DEFAULT 'sem_ajustes',
  aplicado TINYINT(1) NOT NULL DEFAULT 0,
  criado_em DATETIME NOT NULL DEFAULT CURRENT_TIMESTAMP,
  KEY idx_revisao_semanal_usuario (usuario_id, semana_inicio),
  CONSTRAINT fk_rev_sem_usuario FOREIGN KEY (usuario_id) REFERENCES usuarios(id) ON DELETE CASCADE,
  CONSTRAINT fk_rev_sem_plano FOREIGN KEY (plano_id) REFERENCES planos_estudo(id) ON DELETE SET NULL
) ENGINE=InnoDB;
...
CREATE TABLE plano_versoes (
  ...
  tipo_ajuste VARCHAR(80) NULL,
  plano_json LONGTEXT NOT NULL,
  criado_em DATETIME NOT NULL DEFAULT CURRENT_TIMESTAMP,
  UNIQUE KEY uniq_plano_versao (plano_id, usuario_id, versao_numero),
  CONSTRAINT fk_plano_versoes_plano FOREIGN KEY (plano_id) REFERENCES planos_estudo(id) ON DELETE CASCADE,
  CONSTRAINT fk_plano_versoes_usuario FOREIGN KEY (usuario_id) REFERENCES usuarios(id) ON DELETE CASCADE
) ENGINE=InnoDB;

-- ============================================================
-- Exercicios, revisoes por erro e simulados baseados no banco
-- ============================================================

CREATE TABLE exercicios_planejados (
  id INT AUTO_INCREMENT PRIMARY KEY,
  usuario_id INT NOT NULL,
  plano_id INT NULL,
  tarefa_id INT NULL,
  ...
  materia VARCHAR(100) NULL,
  conteudo VARCHAR(180) NULL,
  nivel VARCHAR(40) NOT NULL DEFAULT 'misto',
  quantidade INT NOT NULL DEFAULT 0,
  exercicios_json LONGTEXT NOT NULL,
  origem ENUM('banco','manual') NOT NULL DEFAULT 'banco',
  status ENUM('ativo','arquivado','concluido') NOT NULL DEFAULT 'ativo',
  criado_em DATETIME NOT NULL DEFAULT CURRENT_TIMESTAMP,
  atualizado_em DATETIME NULL DEFAULT NULL ON UPDATE CURRENT_TIMESTAMP,
  KEY idx_exercicios_usuario_semana (usuario_id, semana_inicio, status),
  KEY idx_exercicios_tarefa (tarefa_id, status),
  CONSTRAINT fk_exercicios_usuario FOREIGN KEY (usuario_id) REFERENCES usuarios(id) ON DELETE CASCADE,
  CONSTRAINT fk_exercicios_plano FOREIGN KEY (plano_id) REFERENCES planos_estudo(id) ON DELETE SET NULL,
  CONSTRAINT fk_exercicios_tarefa FOREIGN KEY (tarefa_id) REFERENCES tarefas_estudo(id) ON DELETE SET NULL
) ENGINE=InnoDB;

CREATE TABLE respostas_exercicios_planejados (
  id INT AUTO_INCREMENT PRIMARY KEY,
  usuario_id INT NOT NULL,
  ...
  respondido_em DATETIME NOT NULL DEFAULT CURRENT_TIMESTAMP,
  UNIQUE KEY uniq_resposta_exercicio (usuario_id, exercicio_planejado_id, exercise_key),
  KEY idx_resp_exercicios_usuario (usuario_id, respondido_em),
  CONSTRAINT fk_resp_exercicios_usuario FOREIGN KEY (usuario_id) REFERENCES usuarios(id) ON DELETE CASCADE,
  CONSTRAINT fk_resp_exercicios_lote FOREIGN KEY (exercicio_planejado_id) REFERENCES exercicios_planejados(id) ON DELETE CASCADE
) ENGINE=InnoDB;

CREATE TABLE revisoes_conteudo_ia (
  id INT AUTO_INCREMENT PRIMARY KEY,
  usuario_id INT NOT NULL,
  tarefa_id INT NULL,
  plano_id INT NULL,
  ...
  origem ENUM('banco','manual') NOT NULL DEFAULT 'banco',
  criado_em DATETIME NOT NULL DEFAULT CURRENT_TIMESTAMP,
  KEY idx_revisoes_conteudo_usuario (usuario_id, criado_em),
  KEY idx_revisoes_conteudo_tarefa (tarefa_id),
  CONSTRAINT fk_revisoes_conteudo_usuario FOREIGN KEY (usuario_id) REFERENCES usuarios(id) ON DELETE CASCADE,
  CONSTRAINT fk_revisoes_conteudo_tarefa FOREIGN KEY (tarefa_id) REFERENCES tarefas_estudo(id) ON DELETE SET NULL,
  CONSTRAINT fk_revisoes_conteudo_plano FOREIGN KEY (plano_id) REFERENCES planos_estudo(id) ON DELETE SET NULL
) ENGINE=InnoDB;
...
CREATE TABLE simulado_questoes (
  ...
  KEY idx_simulado_questoes_ordem (simulado_id, ordem),
  CONSTRAINT fk_simulado_questoes_simulado FOREIGN KEY (simulado_id) REFERENCES simulados(id) ON DELETE CASCADE,
  CONSTRAINT fk_simulado_questoes_questao FOREIGN KEY (questao_id) REFERENCES questoes(id) ON DELETE CASCADE
) ENGINE=InnoDB;
...
CREATE TABLE simulado_tentativas (
  ...
  KEY idx_tentativas_usuario (usuario_id, status, iniciado_em),
  CONSTRAINT fk_tentativas_usuario FOREIGN KEY (usuario_id) REFERENCES usuarios(id) ON DELETE CASCADE,
  CONSTRAINT fk_tentativas_simulado FOREIGN KEY (simulado_id) REFERENCES simulados(id) ON DELETE CASCADE
) ENGINE=InnoDB;
...
CREATE TABLE simulado_respostas (
  ...
  acertou TINYINT(1) NOT NULL DEFAULT 0,
  respondido_em DATETIME NOT NULL DEFAULT CURRENT_TIMESTAMP,
  UNIQUE KEY uniq_simulado_resposta (tentativa_id, questao_id),
  CONSTRAINT fk_sim_resp_tentativa FOREIGN KEY (tentativa_id) REFERENCES simulado_tentativas(id) ON DELETE CASCADE,
  CONSTRAINT fk_sim_resp_questao FOREIGN KEY (questao_id) REFERENCES questoes(id) ON DELETE CASCADE
) ENGINE=InnoDB;

CREATE TABLE simulados_planejados (
  id INT AUTO_INCREMENT PRIMARY KEY,
  usuario_id INT NOT NULL,
  plano_id INT NULL,
  tarefa_id INT NULL,
  ...
  status ENUM('bloqueado','liberado','iniciado','finalizado','arquivado') NOT NULL DEFAULT 'bloqueado',
  origem ENUM('banco','manual') NOT NULL DEFAULT 'banco',
  criado_em DATETIME NOT NULL DEFAULT CURRENT_TIMESTAMP,
  atualizado_em DATETIME NULL DEFAULT NULL ON UPDATE CURRENT_TIMESTAMP,
  KEY idx_sim_plan_usuario_status (usuario_id, status, data_liberacao),
  KEY idx_sim_plan_tarefa (tarefa_id),
  CONSTRAINT fk_sim_plan_usuario FOREIGN KEY (usuario_id) REFERENCES usuarios(id) ON DELETE CASCADE,
  CONSTRAINT fk_sim_plan_plano FOREIGN KEY (plano_id) REFERENCES planos_estudo(id) ON DELETE SET NULL,
  CONSTRAINT fk_sim_plan_tarefa FOREIGN KEY (tarefa_id) REFERENCES tarefas_estudo(id) ON DELETE SET NULL
) ENGINE=InnoDB;

CREATE TABLE simulados_planejados_respostas (
  id INT AUTO_INCREMENT PRIMARY KEY,
  usuario_id INT NOT NULL,
  ...
  avaliacao_json LONGTEXT NULL,
  respondido_em DATETIME NOT NULL DEFAULT CURRENT_TIMESTAMP,
  UNIQUE KEY uniq_resp_sim_plan (usuario_id, simulado_planejado_id, question_key),
  KEY idx_resp_sim_plan_usuario (usuario_id, respondido_em),
  CONSTRAINT fk_resp_sim_plan_usuario FOREIGN KEY (usuario_id) REFERENCES usuarios(id) ON DELETE CASCADE,
  CONSTRAINT fk_resp_sim_plan_simulado FOREIGN KEY (simulado_planejado_id) REFERENCES simulados_planejados(id) ON DELETE CASCADE
) ENGINE=InnoDB;

-- ============================================================
-- Seeds minimos para apresentacao
-- ============================================================

INSERT INTO schema_migrations (versao, descricao)
VALUES ('schema_canonico_2026_05_24', 'Schema unico canonico do prototipo EstudAI');

INSERT INTO materias (id, area, nome, slug) VALUES
(1, 'Matematica e suas Tecnologias', 'Matematica', 'matematica'),
(2, 'Linguagens', 'Portugues', 'portugues'),
(3, 'Ciencias da Natureza', 'Biologia', 'biologia'),
(4, 'Ciencias Humanas', 'Historia', 'historia'),
(5, 'Ciencias Humanas', 'Geografia', 'geografia'),
(6, 'Ciencias da Natureza', 'Quimica', 'quimica'),
(7, 'Ciencias da Natureza', 'Fisica', 'fisica');

INSERT INTO conteudos (materia_id, materia, titulo, slug, ordem) VALUES
(1, 'Matematica', 'Porcentagem', 'porcentagem', 1),
(1, 'Matematica', 'Progressao aritmetica', 'progressao-aritmetica', 2),
(1, 'Matematica', 'Funcao afim', 'funcao-afim', 3),
(1, 'Matematica', 'Probabilidade', 'probabilidade', 4),
(1, 'Matematica', 'Geometria plana', 'geometria-plana', 5),
(2, 'Portugues', 'Interpretacao de texto', 'interpretacao-de-texto', 1),
(2, 'Portugues', 'Concordancia verbal', 'concordancia-verbal', 2),
(2, 'Portugues', 'Crase', 'crase', 3),
(2, 'Portugues', 'Figuras de linguagem', 'figuras-de-linguagem', 4),
(3, 'Biologia', 'Ecologia', 'ecologia', 1),
(3, 'Biologia', 'Citologia', 'citologia', 2),
(3, 'Biologia', 'Genetica', 'genetica', 3),
(4, 'Historia', 'Brasil Colonia', 'brasil-colonia', 1),
(4, 'Historia', 'Revolucao Industrial', 'revolucao-industrial', 2),
(5, 'Geografia', 'Cartografia', 'cartografia', 1),
(5, 'Geografia', 'Climatologia', 'climatologia', 2),
(6, 'Quimica', 'Estequiometria', 'estequiometria', 1),
(6, 'Quimica', 'Ligacoes quimicas', 'ligacoes-quimicas', 2),
(7, 'Fisica', 'Cinematica', 'cinematica', 1),
(7, 'Fisica', 'Energia mecanica', 'energia-mecanica', 2);

-- Questoes importadas de database/questions/estudai_seed_questoes_enem_autoral.sql
...
INSERT INTO simulados (id, titulo, descricao, duracao_min, total_questoes, ativo)
SELECT 1, 'Simulado Base EstudAI', 'Simulado demonstrativo com questoes aprovadas da base propria.', 90, COUNT(*), 1
FROM questoes
WHERE status IN ('aprovada','revisada','aprovado','revisado','ativo');

SET @simulado_ordem := 0;
INSERT INTO simulado_questoes (simulado_id, questao_id, ordem)
SELECT 1, q.id, (@simulado_ordem := @simulado_ordem + 1)
FROM questoes q
WHERE q.status IN ('aprovada','revisada','aprovado','revisado','ativo')
ORDER BY q.id;

INSERT INTO conquistas (codigo, nome, descricao, icone, categoria, requisito) VALUES
('streak_3', 'Ritmo inicial', 'Estude por 3 dias.', 'flame', 'streak', 3),
('streak_7', 'Semana consistente', 'Estude por 7 dias.', 'calendar', 'streak', 7),
('streak_30', 'Constancia forte', 'Estude por 30 dias.', 'trophy', 'streak', 30),
('streak_100', 'Longa jornada', 'Estude por 100 dias.', 'award', 'streak', 100),
('questoes_50', 'Primeiras 50', 'Responda 50 questoes.', 'list-checks', 'questoes', 50),
('questoes_200', 'Volume bom', 'Responda 200 questoes.', 'target', 'questoes', 200),
('questoes_500', 'Treino pesado', 'Responda 500 questoes.', 'dumbbell', 'questoes', 500),
('questoes_1000', 'Mil questoes', 'Responda 1000 questoes.', 'medal', 'questoes', 1000),
('acertos_100', 'Cem acertos', 'Acerte 100 questoes.', 'check-circle', 'desempenho', 100),
('perfecto_dia', 'Dia perfeito', 'Finalize um dia com 20 acertos e nenhum erro.', 'sparkles', 'desempenho', 20),
('simulado_80', 'Simulado 80+', 'Faca 80 por cento ou mais em um simulado.', 'badge-check', 'desempenho', 80),
('meta_semana', 'Meta batida', 'Conclua uma meta semanal.', 'flag', 'especial', 1),
('primeiro_dia', 'Primeiro dia', 'Finalize o primeiro dia de estudo.', 'sun', 'especial', 1),
('primeiro_simulado', 'Primeiro simulado', 'Finalize o primeiro simulado.', 'clipboard-check', 'especial', 1),
('anotador', 'Anotador', 'Crie sua primeira anotacao.', 'notebook-pen', 'especial', 1),
('revisador', 'Revisor', 'Refaca uma questao que ja errou.', 'rotate-ccw', 'especial', 1),
('questoes_100', '100 questoes respondidas', 'Responda 100 questoes.', 'target', 'questoes', 100),
('primeira_semana_completa', 'Primeira semana completa', 'Conclua todas as tarefas validas da semana.', 'calendar', 'especial', 1),
('semana_sem_atrasos', 'Semana sem atrasos', 'Mantenha a semana atual sem tarefas atrasadas.', 'check', 'especial', 1),
('revisou_10_erros', 'Revisou 10 erros', 'Revise pelo menos 10 erros reais.', 'refresh', 'especial', 10),
('plano_semanal_concluido', 'Plano semanal concluido', 'Conclua um plano semanal salvo.', 'clipboard', 'especial', 1),
('primeira_redacao', 'Primeira redacao', 'Salve ou analise sua primeira redacao ENEM.', 'pencil', 'especial', 1);
